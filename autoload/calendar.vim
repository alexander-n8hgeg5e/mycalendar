...
let g:calendar_diary = "~/PIM/diary"
...
let cnr = col('.') - (col('.')%(24+5)) + 1
  ...
  " call the action function
   call calendar#diary(day, month, year, week, dir)
endfunc
...
function! calendar#show(...)
  ...
  if getbufvar(vwinnum, 'Calendar') == 'Calendar'
    if bufwinnr(vwinnum) > -1
     let vwinnum = bufwinnr(vwinnum)
    else
    "echo 'other tab?... what now?'
    endif
  else
    let vwinnum = -1
  endif
  if vwinnum >= 0
    ...
    let nontext_columns = 5
    if dir
      silent execute 'bo '.vheight.'split __Calendar'
      setlocal winfixheight
    else
      silent execute 'to '.(vcolumn+nontext_columns).'vsplit __Calendar'
  ...
  if exists('g:calendar_navi')
    exec "silent! syn match CalNavi display \"\\(<"
        \.get(split('navilabel', ','), 0, '')."\\|"
        \.get(split('navilabel', ','), 2, '').">\\)\""
    exec "silent! syn match CalNavi display \"\\s"
        \.get(split('navilabel', ','), 1, '')."\\s\"hs=s+1,he=e-1"
  endif
...
function! calendar#diary(day, month, year, week, dir)
  ...
  exe "noremap <buffer> <CR> :call calendar#saveandsync()<CR>"
  exe "noremap <buffer> <Space> :call calendar#save()<CR>"
endfunc

function! calendar#save()
write
"let g:currentshell = &shell
"set shell=/bin/bash
"silent execute '!cd %:p:h ;git add %:p:t; git commit --no-status -m"autocommit: vim save file" %:p:t'
"let &shell=g:currentshell
wincmd p
endfunction
function! calendar#saveandsync()
write
"let g:currentshell = &shell
"set shell=/bin/bash
call RunJob( 'calendarsync' )
"silent !calendarsync
"let &shell=g:currentshell
wincmd p
endfunction

function! calendar#showdiary(day, month, year, week, dir)
  " build the file name and create directories as needed
  if !isdirectory(expand(g:calendar_diary))
    call confirm("please create diary directory : ".g:calendar_diary, 'OK')
    return
...
function! s:CalendarBuildKeymap(dir, vyear, vmnth)
  ...
  nnoremap <silent> <buffer> <Plug>CalendarShowDiary  :call calendar#showdiary()<cr>
  ...
  execute 'nnoremap <silent> <buffer> <Plug>CalendarGotoPrevMonth :call calendar#action("p")<cr>'
  execute 'nnoremap <silent> <buffer> <Plug>CalendarGotoNextMonth :call calendar#action("n")<cr>'
  ...
  execute 'nmap <buffer> ' . s:calendar_keys['do_action2'] . ' <Plug>CalendarDoAction'
